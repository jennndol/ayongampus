...
class Progress(models.Model):
    ...
    Data stored in csv using the format:
        subject, score, possible, subject, score, possible, ...
    """
    ...
    def list_all_cat_scores(self):
        """
        Returns a dict in which the key is the subject name and the item is
        ...
        The dict will have one key for every subject that you have defined
        """
        score_before = self.score
        output = {}

        for cat in Subject.objects.all():
            to_find = re.escape(cat.subject) + r",(\d+),(\d+),"
            ...
            if match:
                ...
                output[cat.subject] = [score, possible, percent]

            else:  # if subject has not been added yet, add it.
                self.score += cat.subject + ",0,0,"
                output[cat.subject] = [0, 0]

        if len(self.score) > len(score_before):
            # If a new subject has been added, save changes.
            self.save()
    ...
    def update_score(self, question, score_to_add=0, possible_to_add=0):
        ...
        """
        chapter_test = Subject.objects.filter(chapter=question.chapter) \
            .exists()

        if any([item is False for item in [chapter_test,
                                           score_to_add,
                                           possible_to_add,
                                           isinstance(score_to_add, int),
                                           isinstance(possible_to_add, int)]]):
            return _("error"), _("chapter does not exist or invalid score")

        to_find = re.escape(str(question.chapter)) + \
...
class Question(models.Model):
    ...
    def __str__(self):
        return self.content + " --- " + self.chapter.chapter + "."
