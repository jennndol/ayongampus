...
from ayongampus.activities.models import Notification
...
class Profile(models.Model):
    ...
    def get_picture(self):
        no_picture = '/static/assets/img/user.png'
        try:
    ...
    def get_suggestions(self):
        return User.objects.exclude(
            id__in=set([following.following_id for following in self.user.who_is_following.all()])).exclude(
            id=self.user.id).order_by('?')

    def get_followers(self):
        return User.objects.filter(
            id__in=set([follower.user_id for follower in self.user.who_is_followed.all()])).exclude(
            id=self.user.id).order_by('username')
